...
Task.prototype.test = function (cb) {
  ...
  if (self.form)
    req = {method: 'POST', url: self.url, form: self.form};
  if (self.timeout)
...
Task.prototype.run = function (opt, cb) {
  ...
  var self = this;
  var ctx = {url: this.url, form: this.form, callback: this.callback, timeout: this.timeout, retry: this.retry, force: this.force};
  ...
  if (ctx.form)
    req = {method: 'POST', url: ctx.url, form: ctx.form};
  if (ctx.timeout)
...
Task.prototype.toJSON = function () {
  ...
  ['name', 'url', 'form', 'callback', 'active',
   'cron', 'mask', 'drop', 'timeout', 'retry', 'force',
   'patch', 'urlTemplate', 'callbackTemplate',
