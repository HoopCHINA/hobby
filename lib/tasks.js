...
db.all(function (err, data) {
  ...
  data.keys.forEach(function (k) {
    var task = tasks[k] = new Task(data[k]);
    task.on('change', function () { db.put(this.name, this); });
  });
});
...
Task.prototype.run = function (cb) {
  ...
  var self = this;
  var ctx = {timeout: this.timeout, retry: this.retry, url: this.url, callback: this.callback};
  var req = {url: ctx.url};
